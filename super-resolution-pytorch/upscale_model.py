...
def train(model, dataloader, criterion, optimizer, epoch, device, depth_loss=False, distance=70):
    model.train()
    train_loss = 0.0
    ...
    print(f"Epoch {epoch}, Trainloss: {average_loss:.4f}")
    return average_loss

def test(model, dataloader, criterion, epoch, device, depth_loss=False, distance=70):
    model.eval()
    test_loss = 0.0
    with torch.no_grad():
        for high_res, low_res, low_up, side in dataloader:
        ...
        print(f"Epoch {epoch}, Testloss: {average_loss:.4f}")
